...
# Written by Torsten Seemann <torsten@seemann.id.au>
#
...
my @look =
  ( $ARGV[0], $ARGV[1], "$dir/$STEM.fasta", "$dir/$STEM.singletons.seq" );
...
sub setOptions {
    ...
    @Options = (
        ...
        },
        {
            ...
            DEFAULT => '',
            DESC    =>
              "Directory to put output (default=minimus_YYYY-MM-DDThh:mm:ss)"
        },
        {
            OPT     => "c|clean!",
            VAR     => \$clean,
            DEFAULT => 0,
            DESC    => "Clean excess output from --dir"
        },
        {
            OPT     => "o|overlap=i",
            VAR     => \$overlap,
            DEFAULT => 40,
            DESC    => "Minimum overlap"
        },
        {
            OPT     => "e|conserr=f",
            VAR     => \$conserr,
            DEFAULT => 0.06,
            DESC    => "Maximum consensus error"
        },
        {
            OPT     => "m|minid=i",
            VAR     => \$minid,
            DEFAULT => 94,
            DESC    => "Minimum overlap \%id for align."
        },
        {
            OPT     => "t|maxtrim=i",
            VAR     => \$maxtrim,
            DEFAULT => 20,
            DESC    => "Maximum sequence trimming length"
        },
    );
